...
url = 'http://192.168.218.133/cam-hi.jpg'  # Replace with your ESP32 IP address
...
def tts_announcement(detected_objects):
...
def findObjects(outputs, img):
    ...
    # Announce detected objects using threading to avoid lag
    if detected_objects:
        detected_objects = list(set(detected_objects))  # Remove duplicates
        tts_thread = threading.Thread(target=tts_announcement, args=(detected_objects,))
        tts_thread.start()
...
while True:
    try:
        ...
        # Add a slight delay to allow the camera to process new frames
        cv2.waitKey(10)
